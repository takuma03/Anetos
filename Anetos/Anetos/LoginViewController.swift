                ...
                for (key,subJson):(String, JSON) in json["id"] {
                    ...
                    DispatchQueue.main.async {
                        ...
                        let task = URLSession.shared.dataTask(with: request, completionHandler: {
                            ...
                            //配列にレスポンスデータを格納
                            //要素数分ループして配列に格納する
                            if json["large_class"].stringValue.contains("トップス"){
                                self.appDelegate.tops_cloth_array.append(subJson["cloth_id"].stringValue)
                            } else if json["large_class"].stringValue.contains("その他"){
                                self.appDelegate.others_cloth_array.append(subJson["cloth_id"].stringValue)
                            } else {
                                self.appDelegate.bottoms_cloth = subJson["cloth_id"].stringValue
                            }
